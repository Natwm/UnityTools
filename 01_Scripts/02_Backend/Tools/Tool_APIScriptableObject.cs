...
namespace Blacktool.Backend
{
    ...
    public class Tool_APIScriptableObject : ScriptableObject
    {
        ...
        [Header("Dev data")] [SerializeField] private string apiKeyDev = "t84Gtj6S5FDCXsnydmrVfSQ6";
        [SerializeField] private string urlDev;

        [Header("Prod Data")] [SerializeField] private string apiKeyProd = "t84Gtj6S5FDCXsnydmrVfSQ6";
    ...
    [SerializeField] private bool verboseLog;
        ...
        [Header("Route")] private List<ApiRoad> listOfRoad_Dev;
        ...
        public string GetBackendRoad(BackRoad askedRoad)
        {
            if (m_IsProdApi)
            {
                string road = backendRoad_Prod.ContainsKey(askedRoad)
                    ? backendRoad_Prod[askedRoad]
                    : "There isn't a road setup in the scriptableObject";
                return urlProd + road;
            }
            else
            {
                string road = backendRoad_Dev.ContainsKey(askedRoad)
                    ? backendRoad_Dev[askedRoad]
                    : "There isn't a road setup in the scriptableObject";
                return urlDev + road;
            }
        }
